#!/usr/bin/env bash
# Install the WordPress test library + a throwaway test database for the
# integration suite. Canonical WP-CLI scaffold installer.
#
...
install_wp() {
	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		if [ "$WP_VERSION" == 'latest' ]; then
			...
			if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
				...
				# otherwise, scan the releases and get the most recent .x.x version of the major release.
	...
	# No custom db.php drop-in: WordPress core's native mysqli wpdb is correct on
	# PHP 8.x. The old markoheijnen/wp-mysqli drop-in is unmaintained and breaks
	# on modern PHP/WP, so it is deliberately omitted.
}
